...
from src.models.bucket import BucketImageSettings


class ProcessImage:
    def __init__(self, file_path: str, folder_path:str):
        self.image_path = Path(file_path)
        self.img = Image(filename=self.image_path)
        self.ext = str(self.image_path).split(".")[-1]
        self.new_path = Path(folder_path)

    def resize_image(self, width: int, height: int) -> Self:
        self.img.resize(width=width, height=height, filter="lanczos")
        return self
...
def process_image(result: BucketImageSettings, path: str, folder_path:str) -> str:
    
    current_process_image = ProcessImage(
        file_path=path,
